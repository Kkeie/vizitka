...
describe("POST /api/auth/register — создать новый аккаунт", () => {
  it("TC-AUTH-01: успешная регистрация — письмо с подтверждением, JWT выдаётся только после verify", async () => {
...
describe("POST /api/auth/login — войти существующим пользователем", () => {
  ...
  it("TC-AUTH-08b: верный пароль, но email не подтверждён — 403 email_not_verified", async () => {
    const u = `noverify_${Date.now()}`;
    const email = `${u}@example.test`;
