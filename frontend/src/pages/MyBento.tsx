...
export default function MyBento() {
  const [state, setState] = React.useState<{ loading: boolean; name?: string; bio?: string | null; avatarUrl?: string | null; blocks?: any[]; username?: string; error?: string }>({ loading: true });
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        setState({ loading: false, name: data.name, bio: data.bio, avatarUrl: data.avatarUrl, blocks: data.blocks, username: uname });
      } catch {
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 48, flexWrap: "wrap", gap: 24 }}>
            ...
            <button
              ...
              onClick={async () => {
                ...
                const url = `${window.location.origin}/${state.username}`;
                try { 
