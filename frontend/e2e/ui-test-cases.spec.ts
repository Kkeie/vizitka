...
test.describe("Регистрация и вход (как в инструкции для пользователя)", () => {
  test("TC-UI-REG-01: новый человек вводит ник и пароль — попадает в редактор, «пропуск» лежит в sessionStorage", async ({ page }) => {
    const u = `uitest_01_${Date.now()}`;
    await page.goto("/register");
    await page.getByPlaceholder("Придумайте username").fill(u);
    await page.getByPlaceholder("Придумайте password").fill("password123");
  ...
  test("TC-UI-REG-02: слишком короткий ник — остаёмся на странице регистрации и видим подсказку про 3 символа", async ({ page }) => {
    await page.goto("/register");
    await page.getByPlaceholder("Придумайте username").fill("ab");
    await page.getByPlaceholder("Придумайте password").fill("password123");
    await page.getByRole("button", { name: /создать аккаунт/i }).click();
  ...
  test("TC-UI-REG-03: короткий пароль — подсказка про 4 символа, в редактор не пускает", async ({ page }) => {
    await page.goto("/register");
    await page.getByPlaceholder("Придумайте username").fill(`uitest_03_${Date.now()}`);
    await page.getByPlaceholder("Придумайте password").fill("abc");
    await page.getByRole("button", { name: /создать аккаунт/i }).click();
    await expect(page).not.toHaveURL(/\/editor/);
  ...
  test("TC-UI-REG-04: ник уже занят — под полем появляются варианты @..., по клику подставляется другой ник", async ({ page, request }) => {
    ...
    await page.goto("/register");
    await page.getByPlaceholder("Придумайте username").fill(taken);
    ...
    const val = await page.getByPlaceholder("Придумайте username").inputValue();
  ...
  test("TC-UI-LOGIN-01: существующий пользователь входит — открывается редактор", async ({ page, request }) => {
    ...
    await page.getByPlaceholder("Введите username").fill(u);
    await page.getByPlaceholder("Введите password").fill("password123");
  ...
  test("TC-UI-LOGIN-02: неверный пароль — красная ошибка, остаёмся на странице входа", async ({ page, request }) => {
    ...
    await page.goto("/login");
    await page.getByPlaceholder("Введите username").fill(u);
    await page.getByPlaceholder("Введите password").fill("wrongpassword");
    await page.getByRole("button", { name: /войти/i }).click();
    await expect(page).not.toHaveURL(/\/editor/);
    await expect(page.getByText(/неверный username или password/i)).toBeVisible();
  });
