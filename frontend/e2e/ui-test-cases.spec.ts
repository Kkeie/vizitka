...
test.describe("Регистрация и вход (как в инструкции для пользователя)", () => {
  ...
  test("TC-UI-REG-05: на мобильном шаге регистрации виден полный домен ссылки", async ({ page }) => {
    ...
    await page.goto("/register");
    await expect(page.locator(".username-field .prefix")).toHaveText("vizitka.ddns.net/");
  });
...
test.describe("Квадратность блоков сетки при загрузке", () => {
  async function createSquareTestUser(request: Parameters<Parameters<typeof test>[1]>[0]["request"]) {
    const u = `sq_${Date.now()}_${Math.floor(Math.random() * 1e6)}`;
    const email = `${u}@test.local`;
    const reg = await request.post(`${API}/api/auth/register`, {
      data: { username: u, email, password: "password123" },
    });
      ...
      data: {
        blockSizes: {
          [String(blockId)]: {
            ...
            rowSpan: 1,
            anchorsByBreakpoint: {
              mobile: { gridColumnStart: 1, gridRowStart: 1 },
              tablet: { gridColumnStart: 1, gridRowStart: 1 },
              desktop: { gridColumnStart: 1, gridRowStart: 1 },
            },
          },
        },
    ...
    return { u, token, blockId };
  }

  test("TC-UI-GRID-01: 1×1 блок квадратный сразу при загрузке редактора (desktop), без resize", async ({ page, request }) => {
    await page.setViewportSize({ width: 1400, height: 900 });
    const { token, blockId } = await createSquareTestUser(request);
    ...
    const { width, height } = await card.evaluate((el) => {
      const r = el.getBoundingClientRect();
      return { width: r.width, height: r.height };
    });

    expect(width).toBeGreaterThan(50);
    expect(Math.abs(width - height)).toBeLessThanOrEqual(3);
  });

  test("TC-UI-GRID-02: 1×1 блок квадратный сразу при загрузке редактора (tablet, 3 колонки), без resize", async ({ page, request }) => {
    await page.setViewportSize({ width: 900, height: 900 });
    const { token, blockId } = await createSquareTestUser(request);
  ...
  test("TC-UI-GRID-03: 1×1 блок квадратный сразу при загрузке публичной страницы (desktop), без resize", async ({ page, request }) => {
    await page.setViewportSize({ width: 1400, height: 900 });
    const { u, blockId } = await createSquareTestUser(request);
